...
class Linear(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if self.mode == 1:
            ...
            #在deepfm 中
            #模式二有两个输入[sparse_input, dense_input]
            #input_shape= [TensorShape([None, 1, 26]), TensorShape([None, 13])]
            #这里的 kernel的shape 是 13*1 
            self.kernel = self.add_weight(
                'linear_kernel',
    ...
    def call(self, inputs, **kwargs):
        if self.mode == 0:
            ...
            fc =tf.matmul(dense_input,self.kernel)
            linear_logit = fc
        else:
            ...
            #相乘之后N*13的矩阵与13*1的矩阵相乘，变成N*1就是一个数值
            fc = tf.matmul(dense_input,self.kernel)
            print('fc=',fc)
            sum_sparse=reduce_sum(sparse_input, axis=-1, keep_dims=False)
            
            print('sum_sparse=',sum_sparse)
            #sum_sparse也是一个数值，两个数相加
            linear_logit = sum_sparse + fc
        if self.use_bias:
